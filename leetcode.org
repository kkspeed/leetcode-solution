...
#+ATTR_LaTeX: :height 0.4\textwidth :center
  [[./images/790_states.png]]
#+END_CENTER

** Domino and Tromino Tiling (#790)
- *Hint 2:* We can classify them into two categories: OneOut and Smooth. How can
  you construct the subproblem for both types?
#+BEGIN_CENTER
#+ATTR_LaTeX: :height 0.4\textwidth :center
  [[./images/790_configure.png]]
#+END_CENTER
...
** Domino and Tromino (#790) Solution
   Notice that smooth_i / oneOut_i only depends on smooth_{i-1}, smooth_{i-2} and
   oneOut_{i-1}.
...
** Frog Jump (#403)
\pause
- *Hint 1:* Whether the frog can reach stone_i depends solely on stones (0..i-1).
\pause
- *Hint 2:* If the frog jumps from stone_j to stone_i, it must have jumped
  from another stone_k to stone_j, where |i - j - (j - k)| <= 1.
\pause
...
#+END_SRC
** Higher Ordered DPs
   I'm calling this type "higher ordered DP" because
   it generally requires O(n^3) to solve. One example of
   it would be matrix multiplication:

   Given matrix chain ABCDEFG, calculate the product of
   it with least number of multiplications.
   Different number of multiplications could be achieved
   by adding parentheses. For example, (A(B(CD)E)(FG)) and
   ((ABCD)(EFG)) may require different number of multiplications.

   Read https://home.cse.ust.hk/~dekai/271/notes/L12/L12.pdf for
   more information on matrix multiplication.

** Burst Balloons (#312)
\pause
- *Hint 1:* If I burst balloons between (i, j) and iand j becomes
  neighbor, assume the sequence is optimal, do any subsequent operations
  affect it?
\pause
- *Hint 2:* The formula:
#+BEGIN_SRC text
max(i, j) = max{max(i, k) + max(k, j) + balloons[i] * balloons[k] * balloons[j]}.
#+END_SRC
...
* Monotonic Stack
** Monotonic Stack
   It's generally used for linear data structure. You maintain a
   (stack-like) structure that is increasing or decreasing. When you
   encounter an element that's smaller / larger than this one, you
   keep popping the stack until the element is larger than / smaller than
   the current top of the stack. In the process of popping, you can do
   various operations on it. Since each element enters the stack and is
   popped off the stack once, it's still linear.

** Min Stack (#155)
\pause
- *Hint 1:* You'll need to keep all the minimum numbers that you encountered
  so far. How do you do that?
\pause
- *Hint 2:* You can use 2 stacks. One regular and one for min numbers. If
  a newly encountered number is smaller than or equal to the one on top of the min
  stack, you push it onto the min stack as well.
...
\pause
- *Hint 2:* You can use a stack, when you encounter an element that's smaller than
  the top of the stack, you keep on popping. Then push the element onto the stack.
  You'll need to keep track of $k$ as well, since you can only pop $k$ times maximum.
...
\pause
- *Hint 1:* You'll need to keep track the elements
  you've encountered so far until you meed a greater element.
  Then assign all the elements that are smaller than that one
  the index. What can you do?
\pause
- *Hint 2:* You can use a stack that's monotonically decreasing.
  When you find an element that's greater than the stack top, you
  keep popping element and assign the index. Just remember the array
  counts circular so you'll need to take care of it.
...
\pause
- *Hint 2:* You can use a monotonically decreasing stack. Pop the stack when you
  see a number that's greater than current value.
...
\pause
- *Hint 1:* For each element, how is the area of the rectangle that includes
  this element is calculated?
#+BEGIN_CENTER
#+ATTR_LaTeX: :height 0.3\textwidth :center
  [[./images/84_maximal_1.png]]
#+END_CENTER
\pause
...
\pause
- *Hint 3:* You can use a monotonic stack to compute the
  Num of rects! Do it from left to right then from right to
  left.
...
\pause
- *Hint 1:* To keep the maximum value for an interval, a stack-like
  structure could be used. But is stack enough?
\pause
- *Hint 2:* You'll need to remove from bottom of stack -- so might be
  using LinkedList.
\pause
- *Hint 3: First, we pop the last element
  from the window. If that is at the bottom of stack, we remove it as well.
  Then for a new element, we keep popping the stack until we find an element
  that's greater than this element. This still takes linear time and the
...
- Fast-slow pointers: 2 pointers go from the same starting point but
  one moves faster and the other moves slower.
- Outbound pointers: 2 pointers start at the same point but move out-bound.

In general, two pointers are used if there is some condition that you
can decide the direction for the pointers to move.
** Linked List Cycle (#141)
\pause
- *Hint:* You can use a fast and a slow pointer. The fast pointer moves 2 nodes
  each time and the slow pointer moves 1 node each time. If the two pointers meet,
  it's the cycle.
...
\pause
- *Hint 1:* When two pointers meet, what's the distance between the two pointers
  traveled?
\pause
- *Hint 2:* The distance is the length of the circle. Then you can ask the fast
  pointer to move such distance. Then move each pointer by 1 unit. What's the
  meet point?
...
** Two Sum II (#167)
\pause
- *Hint:* Start from two sides i, j. If a_i + a_j < target, we should increase i.
  Otherwise, if a_i + a_j > target, we should decrease j.
...
\pause
- *Hint 1:* For a container with 2 sides l_1 and l_2,
  the area is min(height[l_1], height[l_2]) * (l2 - l1).
\pause
- *Hint 2:* Say l_1 is smaller than l_2, what is the next position for l_1
  so that it's area could be greater than current one?
\pause
- *Hint 3:* The next candidate for l_1 can only be those position a_i such that
  height[a_i] > l_1. Otherwise, the height is smaller / unchanged and the length
  is reduced. In such case, the area is always smaller.
...
\pause
- *Hint 1:* For a pair of indices i and j (i < j), if height[i] < height[j]
  and assume everything between i and j is smaller than i,
  it's guaranteed that you can scan from i to j and the total volume would be
  sum(height[i] - height[t]) for t between i and j. Now, if we have j > k > i that
  height[k] > height[i], what do you do?
\pause
- *Hint 2:* For i < k < j, if height[k] > height[i], then if height[k] < height[j],
  you can repeat that from k to j otherwise, you can go from j to k. Thus, you have
  a two pointers algorithm.
...
* Parsing
** Parsing
There are a certain category of problems that require you
to come up with a mini parser. The parser is usually a
recursive descendant parser -- most likely LL(1) parser. So
you'll need to first make sure you understand the grammar.

If the grammar is left-recursive, that is, the grammar's transitive
closure has the form T = T |... You'll need to refactor the grammar
so that it's not left recursive anymore.

Please read context-free grammar at: https://en.wikipedia.org/wiki/LL_parser
...
*Note:* Actually most problems in this section could be solved
in a simpler way than the above steps. But doing so will make the solution
more systematic and it's good exercise. That being said, the solution is
lengthy and you may need to actually come up with a different solution and/or
shorter solution in actual interview (not so hard once you already know the
grammar).
...
\pause
- *Hint 3:* No! It's not left recursive and you can safely write down the
  parser recursively.
...
** Decode String (#394) Solution
   Then we have a couple of inherited classes that represents different types of
   grammar elements:
#+BEGIN_SRC java
...
#+END_SRC
** Decode String (#394) Solution
   Since the grammar writes "S*", we'll need to represent zero or
   more such element. We'll thus need yet another one:
#+BEGIN_SRC java
// Represents S*
...
** Decode String (#394) Solution
   Then we'll define the \tt{ParsingState} structure that represents the
   input stream It's pretty standard across different questions.
#+BEGIN_SRC java
...
** Mini Parser (#385)
\pause
- *Hint 1:* Can you write down the grammar? Is the
...
** Number of Atoms (#726) Solution
   Now we can define our element class. It has an
   atomCount method that returns the atom count to a hashmap.
#+BEGIN_SRC java
interface Element {
...
\pause
- *Hint 2:* One possible form of grarmmar is:
#+BEGIN_SRC text
S = <boolean>(?S:S)?|<Integer>
#+END_SRC
It's not left recursive.
** Ternary Expression Parser (LintCode #887, LeetCode: #439) Solution
   Let's define the parsed result. It has an eval function that evaluates the
   syntax tree:
#+BEGIN_SRC java
interface Term {
    String eval();
